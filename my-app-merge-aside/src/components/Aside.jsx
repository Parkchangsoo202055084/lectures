...
import {
  translateToKorean,
  isEnglishQuery,
} from "../utils/englishKoreanTranslator";
...
const Aside = ({
        ...
        {items.map((item, idx) => {
          ...
          const handleLabelClick = () => {
            if (hasChildren) {
              ...
              const normalizedName = currentItemLabel
                .replace(/\s*([()])\s*/g, "$1")
                .trim();
  ...
  return (
    <>
      {/* PC 사이드바 */}
      {!isMobile && (
        <aside className={styleA.aside}>{renderAsideContent()}</aside>
      )}
      ...
      {isMobile && isSidebarOpen && (
        <>
          ...
          <div className={styleA["mobile-menu-popup"]}>
            <div className={styleA["menu-content"]}>{renderAsideContent()}</div>
          </div>
        </>
      )}
